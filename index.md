...
<div class="kb-header-section">
  <div class="kb-header-row">
    <h1 class="kb-main-title">Yazılar</h1>
  </div>

  <div class="kb-filters-row">
    <button data-topic-filter="all" class="kb-chip kb-chip-active">Tümü</button>
    {% for topic in site.data.topics.topics %}
    <button data-topic-filter="{{ topic.id }}" class="kb-chip">{{ topic.name }}</button>
    {% endfor %}
    <a href="/about/" class="kb-chip kb-chip-link">Hakkımda</a>
  </div>
</div>
...
<div id="topic-sections" class="kb-articles-container">
  {% for topic in topics %}
    {% assign posts_in_topic = site.posts | where: "topic", topic.id | where_exp: "item", "item.lang != 'en'" %}
...
<div id="no-results" class="kb-no-results hidden">
  ...
  <p>Seçilen filtrelere uygun makale bulunamadı.</p>
</div>
